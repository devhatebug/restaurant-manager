...
const TablePrepareOrders = () => {
  const [dataPareOrders, setDataPareOrders] = useState();
  ...
  const alertSuccessPre = () => {
    messageApi.success("Đơn hàng giao thành công");
  };
  const alertErr = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const newOrders = data.filter((el) => el?.statusOrder === "prepare");
      setDataPareOrders(newOrders);
  ...
  const handlePrePare = async (id) => {
    const orderPre = dataPareOrders.find((el) => el.id === id);
    const dataUpdate = {
  ...
  const prepareOrders = async () => {
    ...
    try {
      ...
      const dataSelect = dataPareOrders.filter((el) =>
...
export default TablePrepareOrders;

...
const FormEditProduct = ({onClose, dataProduct, setMiddleCheck, products}) => {
    ...
    const handleUpdateProduct = async () => {
        ...
        if (code === data.codeItem || checkValue === undefined) {
            try {
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center ml-0 sm:ml-[260px] px-[10px]">
            <div className="animate-openingPopup w-[900px] h-[80%] overflow-y-auto bg-white border border-gray-200 rounded-lg shadow-lg px-[20px] py-[20px]">
                {isWarning && <div id="alert-border-2" className="flex items-center p-4 mb-4 text-red-800 border-t-4 border-red-300 bg-red-50" role="alert">
                    <svg className="flex-shrink-0 w-4 h-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5ZM9.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM12 15H8a1 1 0 0 1 0-2h1v-3H8a1 1 0 0 1 0-2h2a1 1 0 0 1 1 1v4h1a1 1 0 0 1 0 2Z"/>
                    </svg>
                    <div className="ms-3 text-sm font-medium">
                        <strong>Cảnh báo lỗi!</strong> Mã sản phẩm đã tồn tại trong hệ thống
                    </div>
                    <button onClick={() => setIsWarning(false)} type="button" className="ms-auto -mx-1.5 -my-1.5 bg-red-50 text-red-500 rounded-lg focus:ring-2 focus:ring-red-400 p-1.5 hover:bg-red-200 inline-flex items-center justify-center h-8 w-8"  data-dismiss-target="#alert-border-2" aria-label="Close">
                ...
                <div className="bg-white">
                    <div className="py-8 px-4 mx-auto max-w-2xl lg:py-16">
                        <h2 className="mb-4 text-xl font-bold text-gray-900">Chỉnh sửa sản phẩm</h2>
                        <div className="grid gap-4 sm:grid-cols-2 sm:gap-6">
                            <div className="sm:col-span-2">
                                <label htmlFor="name" className="block mb-2 text-sm font-medium text-gray-900">Tên sản phẩm</label>
                                <input onChange={(e) => setName(e.target.value)} type="text" name="name" id="name" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder={data.nameItem} required="" />
                            </div>
                            <div className="w-full">
                                <label htmlFor="price" className="block mb-2 text-sm font-medium text-gray-900">Giá gốc</label>
                                <input onChange={(e) => setPrice(e.target.value)} type="text" name="price" id="price" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder={formatPrice(data.price) + 'đ'} required="" />
                            </div>
                            <div className="w-full">
                                <label htmlFor="endow" className="block mb-2 text-sm font-medium text-gray-900">Giảm giá</label>
                                <input onChange={(e) => setEndow(e.target.value)} type="number" name="endow" id="endow" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder={data.endow + `%`} required="" />
                            </div>
                            <div>
                            ...
                            <div className="w-full">
                                ...
                                <input onChange={(e) => setCode(e.target.value)} type="text" name="code" id="code" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder={data.codeItem} required="" />
                            </div>
                            <div className="sm:col-span-2">
                                {isChangeImg && !isImg && <div className="flex items-center justify-center w-full">
                                    <label htmlFor="img-file" className="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100">
                                        ...
                                        <input onChange={handleChangeValueImg} id="img-file" type="file" className="hidden" />
                                    </label>
                                </div>}
                                {!isChangeImg && 
                                    <div className="relative w-full border-2 border-gray-300 border-dashed rounded-lg cursor-pointer p-[5px]">
                                        <button onClick={changeImg} className="absolute bg-rose-600 p-[10px] rounded-lg top-[10px] left-[10px] text-white font-medium ">
                                            Thay đổi
                                        </button>
                                        <img className="" src={blobtoBase64(data.imgItem)} alt="" />
                                    </div>
                                }
                                {isImg && 
                                    <div className="relative w-full border-2 border-gray-300 border-dashed rounded-lg cursor-pointer p-[5px]">
                                        <button onClick={changeNewImg} className="absolute bg-rose-600 p-[10px] rounded-lg top-[10px] left-[10px] text-white font-medium ">
                                            Thay đổi
                                        </button>
                        ...
                        <button onClick={handleUpdateProduct} className="inline-flex items-center px-5 py-2.5 mt-4 sm:mt-6 text-sm text-white font-medium text-center bg-[#0369A1] rounded-lg focus:ring-4 focus:ring-primary-200 hover:bg-primary-800">
                            Cập nhật
                        </button>
                </div>
                </div>

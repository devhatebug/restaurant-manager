...
const FormEditProduct = ({ onClose, dataProduct, setMiddleCheck, products, setAddErr, setAddFinish }) => {
    ...
    const [previewImg, setPreviewImg] = useState(data.imgItem);
    const [isChangeImg, setIsChangeImg] = useState(false);
    const [isLoading, setIsLoading] = useState(false);
    const [isWarning, setIsWarning] = useState(false);
    const [maxSize, setMaxSize] = useState(false);

    // Xử lý ảnh từ server nếu cần
    useEffect(() => {
        if (data.imgItem && !data.imgItem.startsWith('data:image/')) {
            const loadImageAsDataURL = async () => {
                try {
                    const response = await fetch(data.imgItem);
    ...
    const handleUpdateProduct = async () => {
        const checkValue = products.find((product) => product.codeItem === code && product.id !== data.id);
        ...
        try {
            ...
            formData.append('idItem', data.id);

            if (imgFile) {
                formData.append('file', imgFile);
            }

            await axios.put(`http://127.0.0.1:8080/api-menu/update-menu`, formData, {
                headers: {
